...
local modifier_lib = require "script._lib.lib_population_modifiers"
local list_lib = require "script._lib.lib_population_tables"
local ui_lib = require "script._lib.lib_population_ui_tables"
local economic_lib = require "script._lib.lib_population_economics"
local immigration_lib = require "script._lib.lib_population_immigration"
local units_lib = require "script._lib.lib_units"
------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------
--script constants
...
function set_tick_tock(context)
  triggerload = scripting.game_interface:load_named_value("triggerload", "", context);
    if triggerload == "on" then triggerload = "off" return end
    if triggerload == "off" then triggerload = "on" return end
  triggerload = "on"
end
...
function PopLog(text, ftext) -- the context is the function name that it is being used in. Enter as a string.
...
function Debug(text, ftext) -- the context is the function name that it is being used in. Enter as a string.
...
------------------------------------------
-- Region Pop To Log File
-----------------------------------------
--Output region pops to log, requires the name of the region list
...
-- expects a REGION_SCRIPT_INTERFACE as the context 
...
-- changes the pop values for a region the table must contain 4 integers 
  ...
  PopLog("Region Table Set: " .. regionName .." | " .. region_table[regionName][1].. region_table[regionName][2], "SetRegionPop()")
...
function GetCampaignName()
  ...
  for k,v in pairs(campaign_table) do
    if scripting.game_interface:model():campaign_name(k) then
      return k;
    end
  end
end
...
function RegionPopGrowth(region)
  ...
  Debug("region growth start" .. regionName, "RegionPopGrowth()")
  if region_table[regionName] then
    ...
    Debug("pop temp pop table set up" .. regionName, "RegionPopGrowth()")
    -- get toal population
    local totalPopulation = regionPopTable[1] + regionPopTable[2] + regionPopTable[3] + regionPopTable[4]
    -- reset region flags
    if region_flag[regionName][ENEMIES_PRESENT] == true then
      foreignArmies = true
      region_flag[regionName][ENEMIES_PRESENT] = false
    end
    if region_flag[regionName][SETTLEMENT_LOOTED] == true then
      regionLooted = true
      region_flag[regionName][SETTLEMENT_LOOTED] = false
    end 
    if region_flag[regionName][SETTLEMENT_OCCUPIED] == true then
      regionOccupied = true
      region_flag[regionName][SETTLEMENT_OCCUPIED] = false
    end
    Debug("flags reset" .. regionName, "RegionPopGrowth()")
    ...
    Debug("Region pop growth: " .. regionName .. " | level 1", "RegionPopGrowth()")
  -- check if modifiers need to be applied so that they aren't checked 4 times in the for loop
    --2)Food Shortage
    local foodShortage = false
    if region:owning_faction():has_food_shortage() then
      foodShortage = true
    end
    
    --3)Majority Religion & technology
    local majorityCulture = false
    if  region:majority_religion() ==  factionCulture  then
      majorityCulture = true
    end
    --4)Culture/Religion Bonuses
    local baseCultureInTable = false
    if culture_growth_bonus[factionCulture] then
      baseCultureInTable = true
    end
    --7)Faction Capitol
    local regionIsHome = false
    if regionName ==  homeRegion then 
      regionIsHome = true
    end

-- adapted for Divide et Impera

    --8) Province Capitol
    local provCap = false
    if region:building_superchain_exists("SettlementMajor") then
      provCap = true
    end 
    --11) Under siege
    ...
    Debug("region checks done" .. regionName, "RegionPopGrowth()")
    ...
    -- 1) BaseGrowth %
      ...
      Debug("base growth done" .. regionName, "RegionPopGrowth()")
    --2)Food Shortage
      if foodShortage then
        regionPopModTable[i] = regionPopModTable[i] + population_modifier["food_shortage_mod"][i]
      else 
        regionPopModTable[i] = regionPopModTable[i] + population_modifier["not_food_shortage_mod"][i]
      end
      Debug("food growth done" .. regionName, "RegionPopGrowth()")
    --3)Majority Religion & technology
      if majorityCulture then
        ...
        for k, v in pairs(tech_pop_growth_own_culture_table) do
          if faction:has_technology(k) then
            ...
            Debug("tech growth added culture", "RegionPopGrowth()")
          end         
        end 
        ...
        for slots = 0, region:slot_list():num_items() -1 do
          ...
          if slot:has_building() then 
            ...
            Debug(buildingName, "RegionPopGrowth()")
            if building_pop_growth_own_culture_table[buildingName] then
              ...
              Debug("building growth added culture" .. regionName, "RegionPopGrowth()")
            end 
          end 
        end     
      else
        ...
        for k, v in pairs(tech_pop_growth_foreign_culture_table) do
          if faction:has_technology(k) then
            ...
            Debug("tech growth added foreign", "RegionPopGrowth()")
          end 
        end 
        ...
        for slots = 0, region:slot_list():num_items() -1 do
          ...
          if slot:has_building() then 
            ...
            if building_pop_growth_foreign_culture_table[buildingName] then
              ...
              Debug("building growth added foreign culture" .. regionName, "RegionPopGrowth()")
            end 
          end 
        end 

      end
    --  PopLog("building growth done" .. regionName, "RegionPopGrowth()")
    --4)Culture/Religion Bonuses
      if baseCultureInTable then
        regionPopModTable[i] = regionPopModTable[i]  + culture_growth_bonus[factionCulture][i]
        Debug("major culture", "RegionPopGrowth()")
      end
    ...
    --6) Taxation -- tax level returns the percentage points of tax rate so needs to be converted to a decimal to fit with the format of the regionPopModTable values as decimals
      regionPopModTable[i] = regionPopModTable[i]  + ( population_modifier["tax_growth_modifier"][i] * (-0.01 * (taxLevel - 140)))
      Debug("Tax " .. taxLevel, "RegionPopGrowth()")
    --7)Faction Capitol
      Debug("region capital " .. faction:home_region():name(), "RegionPopGrowth()")
      if regionIsHome then
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["faction_capitol_growth_modifier"][i]
        Debug("region capitol " .. regionName , "RegionPopGrowth()") 
      elseif provCap then
      --8) Province Capitol
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["province_capitol_growth_modifier"][i]
        Debug("province capitol " .. regionName, "RegionPopGrowth()")
      end 
      
    --10) Foreign Armies in region
      if foreignArmies then
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["foreign_army_growth_modifier"][i]
        Debug("Foreign army present in region", "RegionPopGrowth()")
      end
    --11) Under siege
      if underSiege then
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["under_siege_growth_modifier"][i] 
        Debug("Garrison under siege " .. regionName, "RegionPopGrowth()")
      end 
    --12) Looted settlement (Raze, looted)
      if regionLooted then
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["looted_growth_modifier"][i]
        Debug("Settlment looted " .. regionName, "RegionPopGrowth()")  
      end
    
    --13) Occupied Settlment
      if regionOccupied then -- and not regionLooted then
        regionPopModTable[i] = regionPopModTable[i]  +  population_modifier["occupied_growth_modifier"][i]  
        Debug("Region occupied " .. regionName, "RegionPopGrowth()")
      end 
    --14) Difficulty difficulty -- not for MP
      if region:owning_faction():is_human() and not CampaignUI.IsMultiplayer() then
    ...
    -- last) apply bundle to regionPopTable table
      ...
      Debug("Region Growth Modifiers: completed " .. regionName .. " pop now: " .. regionPopTable[i] , "RegionPopGrowth()")
    end
  ...
  --  SettlementOccupied 
    ...
    for i =1, 4 do 
      if regionPopTable[i] < population_modifier["base_pop_growth_min_size"][i] then
    ...
    --apply new pop figures to region pop list. Always use this function to update as it checks entries for errors
    SetRegionPop(regionName, regionPopTable[1], regionPopTable[2], regionPopTable[3], regionPopTable[4])  
  --  PrintRegionTable(region_table)
    Debug("Region pop growth: " .. regionName .. " | completed", "RegionPopGrowth()")  
  end
  
end
-- requires faction context
function FactionImmigration(faction)
  ...
  if (factionSize > 1) then
    ...
    for k, v in pairs(technology_desirability) do
      if faction:has_technology(k) then
        for i = 1, 4 do
    ...
    for number = 0, factionSize - 1 do
      ...
      if regionName ==  homeRegion then 
        regionType = 1
      --2) Province Capitol
      elseif region:building_superchain_exists("rom_SettlementMajor") then
        regionType = 2
      else
        for i = 0, region:adjacent_region_list():num_items() -1 do
          local adjacentRegion = region:adjacent_region_list():item_at(i)
          if ( adjacentRegion:owning_faction():name() ~= factionName ) and not adjacentRegion:owning_faction():allied_with(factionName) then
            numNonAllies = numNonAllies + 1
          end
        end
        -- 4) Frontier
        if ( numNonAllies > 0 ) then
          regionType = 4
        --3) Minor
        else
          regionType = 3
      ...
      if faction_immigration_modifiers[factionName] then
        immigrationDesire = immigrationDesire + faction_immigration_modifiers[factionName][regionType]
      else
    immigrationDesire = immigrationDesire + faction_immigration_modifiers["default"][regionType]
      end
      --7) tax_level; people flee to frontiers with high tax to avoid the tax man
      if (taxLevel ~= 100) then
        immigrationDesire = immigrationDesire + immigration_modifiers["tax_level"][regionType]*(taxLevel -100 )/100
      end
      --8) research_queue_idle; stagnant culture will result in people leaving capitals and going to minor regions
      if idleResearch then
      ...
      if (region:majority_religion() ==  factionReligion) then
        immigrationDesire = immigrationDesire + immigration_modifiers["number_of_allies_mult"][regionType]
      end
      ...
      for slots = 0, region:slot_list():num_items() -1 do
        ...
        if slot:has_building() then 
          local buildingName = slot:building():name()
          if building_desirability[buildingName] then
      ...
      local factionClass
      if faction_to_faction_pop_class[factionName] then 
    ...
    for number = 0, factionSize - 1 do
      ...
      for i = 0, numAdjacentRegions -1 do
        local adjacentRegion = region:adjacent_region_list():item_at(i)
        local adjacentName = adjacentRegion:name()
        -- check adjacent region is owned by faction
        if adjacentRegion:owning_faction() == region:owning_faction() then
          ...
          for popClass  = 1, 4 do
            ...
            if  adjacent_region_desire[popClass] > this_region_desire[popClass] then
              ...
              if migrate + population_modifier.base_pop_growth_min_size[popClass] < region_table[regionName][popClass] then
      ...
      if region_sea_additional_adjacent[regionName] then
      --  PopLog("Check sea migration " .. regionName , "FactionImmigration()")
        ...
        for factionRegion = 0, factionRegionList:num_items() - 1 do
        ...
        --  PopLog("Check against " .. checkRegionName , "FactionImmigration()")
          if Contains(region_sea_additional_adjacent[regionName], checkRegionName) then
...
-- calculation and application of region effect bundles
-- expects region event context (not region())
function ApplyRegionBundle(context, cqi)
  local regionName = context:name()
  local regionPop = {0,0,0,0}
  local totalPop = 0
  local factionName = context:owning_faction():name()
  ...
  --PopLog("RegionBundle Initialisation Complete faction set " .. factionCategory , "ApplyRegionBundle()")
  ...
  local up = faction_pop_ratio_eco[factionCategory][1]
  local mid = faction_pop_ratio_eco[factionCategory][2]
  local low = faction_pop_ratio_eco[factionCategory][3]
  local foreign = faction_pop_ratio_eco[factionCategory][4]
  local up_test = regionPop[1]
  local mid_test =  regionPop[2]
  local low_test =  regionPop[3]
  local citizens_test = up_test + mid_test + low_test 
  local citizens = up + mid + low
  local foreign_test = regionPop[4]
  -- Percentage
  local foreignTotalPercent = foreign_test / totalPop
  local upCitizenPercent = up_test / citizens_test
  local midCitizenPercent = mid_test / citizens_test
  local lowCitizenPercent = low_test / citizens_test
 --region states
  
  --foreign %
  -- determine effect bundle set
  if (factionCategory ~= "default") then
--  PopLog("RegionBundle category not default", "ApplyRegionBundle()")
  else
    -- 1) Foreigner % - bundle_1
    -- heartland
    if (foreignTotalPercent < 0.2) then
      bundle_1 = region_state_eco[factionCategory][1][1]
    --Provincial
    elseif (foreignTotalPercent < 0.6) then
      bundle_1 = region_state_eco[factionCategory][1][2]
    --Colony
    elseif (foreignTotalPercent < 0.8) then
  ...
  --  PopLog("RegionBundle bundle_1" .. bundle_1, "ApplyRegionBundle()")
    ...
    --overbearing
    if ( upCitizenPercent > (faction_pop_ratio_eco[factionCategory][1] * 2)) then
      bundle_2 = region_state_eco[factionCategory][2][1]
    --strong
    elseif ( upCitizenPercent > (faction_pop_ratio_eco[factionCategory][1] * 1.2)) then
      bundle_2 = region_state_eco[factionCategory][2][2]
    --weak
    elseif ( upCitizenPercent < (faction_pop_ratio_eco[factionCategory][1] * 0.7)) then
      bundle_2 = region_state_eco[factionCategory][2][3]
    --minimal
    elseif ( upCitizenPercent < (faction_pop_ratio_eco[factionCategory][1] * 0.2)) then
  ...
  --  PopLog("RegionBundle bundle_2" .. bundle_2, "ApplyRegionBundle()")
    ...
    --thriving
    if ( midCitizenPercent > (faction_pop_ratio_eco[factionCategory][2] * 2)) then
      bundle_3 = region_state_eco[factionCategory][3][1]
    --industrious
    elseif ( midCitizenPercent > (faction_pop_ratio_eco[factionCategory][2] * 1.2)) then
      bundle_3 = region_state_eco[factionCategory][3][2]
    --stagnant
    elseif ( midCitizenPercent < (faction_pop_ratio_eco[factionCategory][2] * 0.7)) then
      bundle_3 = region_state_eco[factionCategory][3][3]
    --failing
    elseif ( midCitizenPercent < (faction_pop_ratio_eco[factionCategory][2] * 0.2)) then
  ...
  --extremely_dense
  if ( totalPop > economic_data["pop_extremely_dense"] ) then
    bundle_5 = region_state_eco[factionCategory][5][1]
  --very_dense
  elseif ( totalPop > economic_data["pop_very_dense"]) then
    bundle_5 = region_state_eco[factionCategory][5][2]
  --dense
  elseif ( totalPop > economic_data["pop_dense"]) then
    bundle_5 = region_state_eco[factionCategory][5][3]
  --fairly_dense
  elseif ( totalPop > economic_data["pop_fairly_dense"]) then
    bundle_5 = region_state_eco[factionCategory][5][4]  
  elseif ( totalPop > economic_data["pop_moderate"]) then
    bundle_5 = region_state_eco[factionCategory][5][5]
  elseif ( totalPop > economic_data["pop_sparse"]) then
    bundle_5 = region_state_eco[factionCategory][5][6]
  elseif ( totalPop > economic_data["pop_very_sparse"]) then
    bundle_5 = region_state_eco[factionCategory][5][7]
  elseif ( totalPop > economic_data["pop_extremely_sparse"]) then
...
--  PopLog("RegionBundle cqi" .. cqi, "ApplyRegionBundle()")
  if (bundle_1 ~= "none") then
    scripting.game_interface:apply_effect_bundle_to_characters_force(bundle_1, cqi,-1)
  --  PopLog("RegionBundle bundle_1 applied " .. bundle_1, "ApplyRegionBundle()")
  end
  if (bundle_2 ~= "none") then
    scripting.game_interface:apply_effect_bundle_to_characters_force(bundle_2, cqi,-1)
  --  PopLog("RegionBundle bundle_2 applied" .. bundle_2, "ApplyRegionBundle()")
  end
  if (bundle_3 ~= "none") then
    scripting.game_interface:apply_effect_bundle_to_characters_force(bundle_3, cqi,-1)
  --  PopLog("RegionBundle bundle_3 applied" .. bundle_3, "ApplyRegionBundle()")
  end
  if (bundle_4 ~= "none") then
    scripting.game_interface:apply_effect_bundle_to_characters_force(bundle_4, cqi,-1)
  --  PopLog("RegionBundle bundle_4 applied" .. bundle_4, "ApplyRegionBundle()")
  end
  if (bundle_5 ~= "none") then
    scripting.game_interface:apply_effect_bundle_to_characters_force(bundle_5, cqi,-1)
...
function RemoveRegionBundle(cqi)
  ...
  for k, v in ipairs(region_economic_effect_bundle) do
    scripting.game_interface:remove_effect_bundle_from_characters_force(v, cqi)
  end
end
-- context is region event context
...
-- DOESNT WORK
  ...
  for i = 0, character_list:num_items() - 1 do
    ...
    if  (character_list:item_at(i):region():name() == context:name()) then
      PopLog("Character in region"  , "GetRegionGarrisonCqi()")
      if character_list:item_at(i):character_type("colonel") then
...
local function OnSavingGamePop(context)
  ...
  local army_size = 0
  for k, v in pairs(army_size_cqi) do 
    army_size = army_size + 1
    scripting.game_interface:save_named_value("_pop_army_size_cqi_"..army_size, k, context)   
    scripting.game_interface:save_named_value(k .."_1", army_size_cqi[k][1], context) 
    scripting.game_interface:save_named_value(k .."_2", army_size_cqi[k][2], context)
  ...
  saveFactionCapiitalTable(context)
...
--  assign the table name to the region_table variable
  ...
  loadFactionCapiitalTable(context)
...
---------------- Saving/Loading ----------------
------------------------------------------------
function loadFactionCapiitalTable(context)
  local capitalKeyList = LoadIndexTable(context, "faction_capitol_pops")
  for index, capitalKey in pairs(capitalKeyList) do
    --PopLog("index: " .. index .. ", armyKey: " .. armyKey)
    faction_capital_region_pop[tostring(capitalKey)] = LoadIndexTable(context, "faction_capital_region_pop_" .. capitalKey)
    
  --[[  for key, value in pairs(faction_capital_region_pop[tostring(capitalKey)]) do
...
function loadarmy_size_cqi(context)
  ...
  for index, armyKey in pairs(armyKeyList) do
    ...
    for key, value in pairs(army_size_cqi[tostring(armyKey)]) do
...
function savearmy_size_cqi(context)
  local armyKeyList = {}
  for key, value in pairs(army_size_cqi) do
...
function saveFactionCapiitalTable(context)
  local faction_capital_table = {}
  for key, value in pairs(faction_capital_region_pop) do
    table.insert(faction_capital_table, tostring(key))
    SaveIndexTable(context, faction_capital_region_pop[key], "faction_capital_region_pop_" .. tostring(key))
  end
  
  SaveIndexTable(context, faction_capital_table, "faction_capitol_pops")
end
...
function loadRecruitmentOrdersTable(context)
  ...
  for index, armyKey in pairs(armyKeyList) do
    ...
    for key, value in pairs(recruitmentOrders[tostring(armyKey)]) do
      PopLog("key: " .. key .. ", value: " .. value)
    end
  end
end
...
function saveRecruitmentOrdersTable(context)
  local armyKeyList = {}
  for key, value in pairs(recruitmentOrders) do
    table.insert(armyKeyList, tostring(key))
...
function loadarmyStateRegion(context)
  ...
  for index, armyKey in pairs(armyKeyList) do
    ...
    for key, value in pairs(armyStateRegion[tostring(armyKey)]) do
      PopLog("key: " .. key .. ", value: " .. value)
    end
  end
end
...
function savearmyStateRegion(context)
  local armyKeyList = {}
  for key, value in pairs(armyStateRegion) do
    table.insert(armyKeyList, tostring(key))
...
local function loadArmiesTables(context)
  ...
  for index, armyKey in pairs(armyStateKeyList) do
...
local function saveArmiesTables(context)
  ...
  for key, value in pairs(armyState) do
...
local function OnRegionTurnStartPop(context)
  ...
  PopLog("Region bundle: "  .. context:region():name(), "OnRegionTurnStartPop()")
--  ApplyRegionBundle(context)
end
...
-----------------------------------------------------
  ...
  local characterRegion = character:region():owning_faction():name()
  -- Foreign region 
  -- ENEMIES_PRESENT
  if (characterRegion ~= factionName) then
  ...
  cqi = context:character():cqi()
  if context:character():has_garrison_residence() and  context:character():character_type("colonel") then
    -- check no current effect bundle active in region
    if rom_region_flag[characterRegion][5] == 0 then
    --  RemoveRegionBundle(cqi)
      ApplyRegionBundle(context:character():region(), cqi)
...
local function OnTimeTriggerPop(context) 
  ...
  if ( message == "garrison_occupied") and population_modifier["foreigner_mechanic_on"] == true then
    SetRegionOccupied()
  end 
end

local function HandleRegionCapture(regionName, factionCulture, regionCulture)
  if region_flag[regionName][SETTLEMENT_LOOTED] == true
  then
    if factionCulture == regionCulture
    then
      region_table[regionName][4] = math.ceil((
        region_table[regionName][1] + region_table[regionName][2] + region_table[regionName][3] + region_table[regionName][4]) / 4)
      ...
      region_table[regionName][4] = math.ceil((
        region_table[regionName][1] + region_table[regionName][2] + region_table[regionName][3] + region_table[regionName][4]) / 2)
...
function SetRegionOccupied()
  ...
  for i = 0, faction:character_list():num_items() - 1 do
    if (faction:character_list():item_at(i):cqi() == tempCqi) then
      ...
      --check if the region is a factions starting capital
      if faction_capital_region_pop[regionName] then
        --if the region is a faction capital check if it has been conquered by the region owner or a foreign faction
        if faction_capital_region_pop[regionName][1] == faction:name() then
          -- capital retaken
          -- relative total populations to ensure that new class pop does not exceed current pop
          local totalRegionPop = region_table[regionName][1] + region_table[regionName][2] + region_table[regionName][3] + region_table[regionName][4]
          PopLog("Capital Retaken".. regionName .. " totalRegionPop " .. totalRegionPop, "SetRegionOccupied")
          local previousRegionPop = faction_capital_region_pop[regionName][3] + faction_capital_region_pop[regionName][4] + faction_capital_region_pop[regionName][5] + faction_capital_region_pop[regionName][6] 
          PopLog("Capital Retaken".. regionName .. " previousRegionPop " .. previousRegionPop, "SetRegionOccupied")
          local popSizeModifier = totalRegionPop/previousRegionPop
          PopLog("Capital Retaken".. regionName .. " previousRegionPop " .. previousRegionPop, "SetRegionOccupied")
          local turnsForeignOccupied = faction_capital_region_pop[regionName][2]
          PopLog("Capital Retaken".. regionName .. " turnsForeignOccupied " .. turnsForeignOccupied, "SetRegionOccupied")
          local pop1 = math.floor(faction_capital_region_pop[regionName][3]*popSizeModifier*(1 - population_modifier["faction_capital_turn_reduction"]*turnsForeignOccupied))
          local pop2 = math.floor(faction_capital_region_pop[regionName][4]*popSizeModifier*(1 - population_modifier["faction_capital_turn_reduction"]*turnsForeignOccupied))
          local pop3 = math.floor(faction_capital_region_pop[regionName][5]*popSizeModifier*(1 - population_modifier["faction_capital_turn_reduction"]*turnsForeignOccupied))
          PopLog("Capital Retaken ".. regionName .. " pops " .. pop1 , "SetRegionOccupied")
          PopLog("Capital Retaken ".. regionName .. " pops " .. pop2 , "SetRegionOccupied")
          PopLog("Capital Retaken ".. regionName .. " pops " .. pop3 , "SetRegionOccupied")
          local pop4 = totalRegionPop - pop1 - pop2 - pop3

          PopLog("Capital Retaken ".. regionName .. " " .. pop4, "SetRegionOccupied")
          -- apply pop to region and UI 
          region_table[regionName][1] = pop1
          region_table[regionName][2] = pop2
          region_table[regionName][3] = pop3
          region_table[regionName][4] = pop4
          -- update UIPopulation
          UIPopulation[regionName][1] = pop1
          UIPopulation[regionName][2] = pop2
          UIPopulation[regionName][3] = pop3
          UIPopulation[regionName][4] = pop4
          --reset turn counter
          faction_capital_region_pop[regionName][2] = 0
          PopLog("Capital Retaken".. regionName, "SetRegionOccupied")
          return
        elseif (faction_capital_region_pop[regionName][3] + faction_capital_region_pop[regionName][4] + faction_capital_region_pop[regionName][5]) > 0 then
          -- capital taken from non original owner
          -- region_table[regionName][4] = region_table[regionName][1] + region_table[regionName][2] + region_table[regionName][3] + region_table[regionName][4]
          -- region_table[regionName][1] = 5
          -- region_table[regionName][2] = 10
          -- region_table[regionName][3] = 20
          -- -- update UIPopulation
          -- UIPopulation[regionName][1] = region_table[regionName][1]
          -- UIPopulation[regionName][2] = region_table[regionName][2]
          -- UIPopulation[regionName][3] = region_table[regionName][3]
          -- UIPopulation[regionName][4] = region_table[regionName][4]
          HandleRegionCapture(regionName, factionCulture, regionCulture)
          PopLog("Capital taken from non original owner".. regionName, "SetRegionOccupied")
          return  
        else
          -- capital taken from traditional owner
          -- reset turn counter
          faction_capital_region_pop[regionName][2] = 1
          -- set faction_capital_region_pop[regionName] populations
          faction_capital_region_pop[regionName][3] = region_table[regionName][1]
          faction_capital_region_pop[regionName][4] = region_table[regionName][2]
          faction_capital_region_pop[regionName][5] = region_table[regionName][3]
          faction_capital_region_pop[regionName][6] = region_table[regionName][4]
          -- -- normal pop change
          -- region_table[regionName][4] = region_table[regionName][1] + region_table[regionName][2] + region_table[regionName][3] + region_table[regionName][4]
          -- region_table[regionName][1] = 5
          -- region_table[regionName][2] = 10
          ...
          -- UIPopulation[regionName][4] = region_table[regionName][4]
          HandleRegionCapture(regionName, factionCulture, regionCulture)
          PopLog("Capital taken from original owner".. regionName, "SetRegionOccupied")
          return          
        end
      -- normal region capture
        ...
        -- TODO: insert here
        HandleRegionCapture(regionName, factionCulture, regionCulture)
        PopLog("Non Capital Taken".. regionName, "SetRegionOccupied")
        return
      end
    end 
  end
...
function OnFactionTurnStartPop(context)
  ...
  local low = 0
  local foreign = 0
  for region = 0, context:faction():region_list():num_items() - 1 do
...
--  PopLog("Faction bundle to be applied" .. faction_foreigner_bundle[1], "OnFactionTurnStartPop")
  if (factionForeignRatio > 0.9) then
    scripting.game_interface:apply_effect_bundle(faction_foreigner_bundle[1], factionName, 1)
    PopLog("Faction bundle applied".. faction_foreigner_bundle[1], "OnFactionTurnStartPop")
  elseif (factionForeignRatio > 0.8) then
    scripting.game_interface:apply_effect_bundle(faction_foreigner_bundle[2], factionName, 1)
    PopLog("Faction bundle applied".. faction_foreigner_bundle[2], "OnFactionTurnStartPop")
  elseif (factionForeignRatio > 0.7) then
    scripting.game_interface:apply_effect_bundle(faction_foreigner_bundle[3], factionName, 1)
    PopLog("Faction bundle applied".. faction_foreigner_bundle[3], "OnFactionTurnStartPop")
  elseif (factionForeignRatio > 0.6) then
    scripting.game_interface:apply_effect_bundle(faction_foreigner_bundle[4], factionName, 1)
    PopLog("Faction bundle applied".. faction_foreigner_bundle[4], "OnFactionTurnStartPop")
  elseif (factionForeignRatio > 0.5) then
    scripting.game_interface:apply_effect_bundle(faction_foreigner_bundle[5], factionName, 1)
...
function OnRegionTurnEndPop(context)
  ...
  -- increase capital region occupied counter if not owned by faction_capital_turns_max
  if faction_capital_region_pop[regionName][1] ~= context:region():owning_faction() then
    if faction_capital_region_pop[regionName][2] < population_modifier["faction_capital_turns_max"] then
      faction_capital_region_pop[regionName][2] = faction_capital_region_pop[regionName][2] + 1
    end
  end 
end
------------------------------------------------------------
    ...
    if not isLogAllowed then
      return;
        ...
        if isNew then
                ...
                if not logfile then logfile = io.open("PoR2_MP_log.txt","w") end
        end
  ...
  for building_name = 0, region:slot_list():num_items() - 1 do
      local slot = region:slot_list():item_at(building_name);
        ...
        and Contains(PortBuildingsList, slot:building():name())
         then PopLog("Slot contains building regionHasPort()")
          return true;
        end;
    end PopLog("No Port found regionHasPort()");
  return false;
end;
...
local factions_regions = faction:region_list();
  ...
  for i = 0, factions_regions:num_items() - 1 do
    ...
    if regionName == "" then regionName = faction:home_region():name() end;-- fallback to capital
   return regionName;
end;
...
function isLocalFaction(faction_name)
  faction_name = (type(faction_name) == "string" and faction_name) or ((type(faction_name) == "userdata" and faction_name:name()) or nil);
  if not faction_name then return false end;
...
function UIGetComponent(...)
        local component;
        for arg_num, arg_value in ipairs(arg) do
                if (type(arg_value) == "userdata") and (arg_num == 1) then
                        component = arg_value;
...
local region = UI_data_table.curr_region_name
  ...
  local rp = (UIPopulation and UIPopulation[region]) or (region_table and region_table[region]) or {0,0,0,0}
  for i = 1, 4 do
    UI_data_table.total_pop = (rp[i] or 0) + UI_data_table.total_pop 
  end
end
...
local component = UIGetComponent(unpack(arg));
        if not component then return end;
         ...
         local provinceName = UIProvinceFromRegionname(UI_data_table.curr_region_name)
          ...
          if province_name_table[provinceName][1] == false then
        ...
        local text = population
                if UI_data_table.total_growth > 0.001
                        then text = text .. UI_data_table.col_green_open .. " (+)" ..UI_data_table.col_green_close
                                UIComponent(component):SetStateText(text)
                elseif UI_data_table.total_growth < -0.001 
                        then text = text .. UI_data_table.col_red_open .. " (-)" ..UI_data_table.col_red_close
                                UIComponent(component):SetStateText(text)
                else text = text .. UI_data_table.col_yellow_open .. " (/)" ..UI_data_table.col_yellow_close
                         UIComponent(component):SetStateText(text)
        end
end

-- change province_dev_points to total population value for the selected refion! 
function UIChangeOnTimeTrigger(context)
         UIRegionPopGrowth() 
         UIChangeComponent_province_dev_points("province_dev_points")
end
...
local component = UIGetComponent(unpack(arg));
  ...
  local text = UIComponent(component):GetTooltipText()
     if string.find(text, "Security") or string.find(text, "Your") then 
...
function UIisComponentActive(component)
        if (type(component) ~= "userdata") then return false end;
   ...
   for i = 0, faction:region_list():num_items() - 1 do
    local region = faction:region_list():item_at(i):name()
                for k = 1, 4 do
                  ...
                  if k == 1 then UI_data_table.faction_population_1 = UIPopulation[region][k] + UI_data_table.faction_population_1
                        end

                          if k == 2 then UI_data_table.faction_population_2 = UIPopulation[region][k] + UI_data_table.faction_population_2
                                end

                                  if k == 3 then UI_data_table.faction_population_3 = UIPopulation[region][k] + UI_data_table.faction_population_3
                                        end

                                          if k == 4 then UI_data_table.faction_population_4 = UIPopulation[region][k] + UI_data_table.faction_population_4
                                                end
                end 
        end
...
local faction_name = UIGetLocalFactionName()
  if ui_faction_unique_name_table[faction_name]
      then return ui_faction_unique_name_table[faction_name]
    end
  return "default"
end 
...
function UIGarrisonFactionNameToClass(faction)
  if ui_faction_unique_name_table[faction]
      then return ui_faction_unique_name_table[faction]
    end
  return "default"
end 
...
if not unit_to_pop_table[cardKey] then return 0 end

local class = 0  
-- Litharion
-- AOR addon for Divide et Impera
  if getRecruitableType(cardKey) == "AOR" then
    class = AOR_GetClass(cardKey, factionName)      
  elseif getRecruitableType(cardKey) == "faction" then
...
function getRecruitableType(cardKey)
  if string.find(cardKey, "AOR_") then
    return "AOR" -- If it starts with AOR, it's an AOR unit. These are handled differently
  else return "faction" -- Else it's just a normal recruitable unit
  end
end
...
if not AOR_faction_class_table[faction] then return "default_AOR" end

    if AOR_faction_class_table[faction]
            then return AOR_faction_class_table[faction]
    end
end
...
local classKey = AOR_LocalClass(factionName)
    ...
    for i, unit in pairs (AOR_units_to_faction_class[classKey]) do
       if unit == cardKey  
            then class = unit_to_pop_table[cardKey][1]
        return class
       end
    end
  return class
...
local set_unit_costs = 0 -- default costs are 0
    ...
    if classKey >= 1 and classKey < 5 then 
...
-- Header
text = text.. UI_data_table.total_growth_tx.. line.. line
-- Block
-- set class 1
  local total_noble_growth = UIFormatValuesBehindComma(UI_data_table.total_noble_growth) local predicted_noble_growth_number = UIRemoveValuesBehindComma(UI_data_table.predicted_noble_growth_number)
  if UI_data_table.total_noble_growth > min_value then
    text = text.."I "..noble_name..": "..gr_o.."+"..total_noble_growth.."%".. gr_c.." ("..gr_o.."+"..predicted_noble_growth_number.. gr_c..")"..  line
   elseif UI_data_table.total_noble_growth <= negative_value then
    text = text.."I "..noble_name..": "..r_o..total_noble_growth.."%".. r_c.." ("..r_o..predicted_noble_growth_number.. r_c..")"..  line
   else text = text.."I "..noble_name..": "..y_o..""..total_noble_growth.."%".. y_c.." ("..y_o..""..predicted_noble_growth_number.. y_c..")"..  line
  end
-- class 2
  local total_citizen_growth = UIFormatValuesBehindComma(UI_data_table.total_citizen_growth) local predicted_citizen_growth_number = UIRemoveValuesBehindComma(UI_data_table.predicted_citizen_growth_number) 
  if UI_data_table.total_citizen_growth > min_value then
    text = text.."II "..citizens_name..": "..gr_o.."+"..total_citizen_growth.."%".. gr_c.." ("..gr_o.."+"..predicted_citizen_growth_number.. gr_c..")".. line
   elseif UI_data_table.total_citizen_growth <= negative_value then
    text = text.."II "..citizens_name..": "..r_o..total_citizen_growth.."%".. r_c.." ("..r_o..predicted_citizen_growth_number.. r_c..")"..  line
   else text = text.."II "..citizens_name..": "..y_o.."+"..total_citizen_growth.."%".. y_c.." ("..y_o.."+"..predicted_citizen_growth_number.. y_c..")"..  line
  end
-- class 3
  local total_poor_growth = UIFormatValuesBehindComma(UI_data_table.total_poor_growth) local predicted_poor_growth_number = UIRemoveValuesBehindComma(UI_data_table.predicted_poor_growth_number)  
  if UI_data_table.total_poor_growth > min_value then
    text = text.."III "..poor_name..": "..gr_o.."+"..total_poor_growth.."%".. gr_c.." ("..gr_o.."+"..predicted_poor_growth_number.. gr_c..")".. line
   elseif UI_data_table.total_poor_growth <= negative_value then
    text = text.."III "..poor_name..": "..r_o..total_poor_growth.."%".. r_c.." ("..r_o..predicted_poor_growth_number.. r_c..")"..  line
   else text = text.."III "..poor_name..": "..y_o..""..total_poor_growth.."%".. y_c.." ("..y_o..""..predicted_poor_growth_number.. y_c..")"..  line
  end

local total_foreign_growth = UIFormatValuesBehindComma(UI_data_table.total_foreign_growth) local predicted_foreign_growth_number = UIRemoveValuesBehindComma(UI_data_table.predicted_foreign_growth_number)  
  if UI_data_table.total_foreign_growth > min_value then
    text = text.."IV "..foreign_name..": "..gr_o.."+"..total_foreign_growth.."%".. gr_c.." ("..gr_o.."+"..predicted_foreign_growth_number.. gr_c..")".. line..line
   elseif UI_data_table.total_foreign_growth <= negative_value then
    text = text.."IV "..foreign_name..": "..r_o..total_foreign_growth.."%".. r_c.." ("..r_o..predicted_foreign_growth_number.. r_c..")"..  line..line
   else text = text.."IV "..foreign_name..": "..y_o..""..total_foreign_growth.."%".. y_c.." ("..y_o..""..predicted_foreign_growth_number.. y_c..")"..  line..line
  end

local total_growth_percentage = UIFormatValuesBehindComma(UI_data_table.total_growth_percentage) local total_growth = UIRemoveValuesBehindComma(UI_data_table.total_growth) 
  if UI_data_table.total_growth_percentage > min_value then
    text = text..UI_data_table.total_growth_tx_2..gr_o.."+"..total_growth_percentage.."%".. gr_c.." ("..gr_o.."+"..total_growth.. gr_c..")".. line
   elseif UI_data_table.total_growth_percentage <= negative_value then
    text = text..UI_data_table.total_growth_tx_2..r_o..total_growth_percentage.."%".. r_c.." ("..r_o..total_growth.. r_c..")"..  line
   else text = text..UI_data_table.total_growth_tx_2..y_o..""..total_growth_percentage.."%".. y_c.." ("..y_o..""..total_growth.. y_c..")"..  line
  end
...
if minimum_growth > 0 then text = text .. line end
if UI_data_table.minimum_growth[1] > 0 then text = text .. noble_name..": "..gr_o.."+".. UI_data_table.minimum_growth[1].. gr_c .. " settlers" .. line end
if UI_data_table.minimum_growth[2] > 0 then text = text .. citizens_name..": "..gr_o.."+".. UI_data_table.minimum_growth[2].. gr_c .. " settlers".. line end
if UI_data_table.minimum_growth[3] > 0 then text = text .. poor_name..": "..gr_o.."+".. UI_data_table.minimum_growth[3].. gr_c .. " settlers".. line end
if UI_data_table.minimum_growth[4] > 0 then text = text .. foreign_name..": "..gr_o.."+".. UI_data_table.minimum_growth[4].. gr_c .. " settlers".. line end
...
-- Header, set header first then get all class values
    if UI_data_table.total_base_growth > min_value or UI_data_table.total_base_growth <= negative_value then
...
-- start line 1 class 1
    local base_1 = UIFormatValuesBehindComma(UI_data_table.base_growth[1]) 
            if UI_data_table.base_growth[1] > min_value then  
                    text = text.."I:     ".. gr_o .."+".. base_1.."%    "  .. gr_c 
             elseif UI_data_table.base_growth[1] <= negative_value then
                    text = text.."I:     ".. r_o .. base_1.."%    "  ..r_c 
             else text = text.."I:     ".. y_o .. base_1.."%    "  ..y_c
            end
-- class 2    
    local base_2 = UIFormatValuesBehindComma(UI_data_table.base_growth[2]) 
            if UI_data_table.base_growth[2] > min_value then 
                    text = text.."    II:   ".. gr_o .."+".. base_2.."%"  .. gr_c.. line
             elseif UI_data_table.base_growth[2] <= negative_value then
                    text = text.."    II:   ".. r_o .. base_2.."%"  ..r_c.. line
             else text = text.."    II:   ".. y_o .. base_2.."%"  ..y_c.. line
            end
-- next line
-- class 3    
    local base_3 = UIFormatValuesBehindComma(UI_data_table.base_growth[3]) 
            if UI_data_table.base_growth[3] > min_value then 
                    text = text.."III:   ".. gr_o .."+".. base_3.."%     " ..gr_c 
             elseif UI_data_table.base_growth[3] <= negative_value then
                    text = text.."III:   ".. r_o .. base_3.."%     "  ..r_c 
             else text = text.."III:   ".. y_o .. base_3.."%     "  ..y_c
            end
-- class 4    
    local base_4 = UIFormatValuesBehindComma(UI_data_table.base_growth[4]) 
            if UI_data_table.base_growth[4] > min_value then 
                    text = text.."  IV:   ".. gr_o .."+".. base_4.."%"  .. gr_c .. line
             elseif UI_data_table.base_growth[4] <= negative_value then
                    text = text.."  IV:   ".. r_o .. base_4.."%"  ..r_c .. line
             else text = text.."  IV:   ".. y_o .. base_4.."%"  ..y_c .. line
            end
    end
-- set faction_capital next
-- Header, set header first then get all class values
                if UI_data_table.total_faction_capital > min_value or UI_data_table.total_faction_capital <= negative_value then
...
-- start line 1 class 1
                local faction_capital_1 = UIFormatValuesBehindComma(UI_data_table.faction_capital[1]) 
                        if UI_data_table.faction_capital[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. faction_capital_1.."%    "  .. gr_c 
                         elseif UI_data_table.faction_capital[1] <= negative_value then
                          text = text.."I:     ".. r_o .. faction_capital_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. faction_capital_1.."%    "  ..y_c
                        end
-- class 2              
                local faction_capital_2 = UIFormatValuesBehindComma(UI_data_table.faction_capital[2]) 
                        if UI_data_table.faction_capital[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. faction_capital_2.."%"  .. gr_c.. line
                         elseif UI_data_table.faction_capital[2] <= negative_value then
...
-- class 3              
                local faction_capital_3 = UIFormatValuesBehindComma(UI_data_table.faction_capital[3]) 
                        if UI_data_table.faction_capital[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. faction_capital_3.."%     " ..gr_c 
                         elseif UI_data_table.faction_capital[3] <= negative_value then
                          text = text.."III:   ".. r_o .. faction_capital_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. faction_capital_3.."%     "  ..y_c
                        end
-- class 4              
                local faction_capital_4 = UIFormatValuesBehindComma(UI_data_table.faction_capital[4]) 
                        if UI_data_table.faction_capital[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. faction_capital_4.."%"  .. gr_c .. line
                         elseif UI_data_table.faction_capital[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. faction_capital_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. faction_capital_4.."%"  ..y_c .. line
                        end
                end
-- set province_capital next
-- Header, set header first then get all class values
                if UI_data_table.total_province_capital > min_value or UI_data_table.total_province_capital <= negative_value then
...
-- start line 1 class 1
                local province_capital_1 = UIFormatValuesBehindComma(UI_data_table.province_capital[1]) 
                        if UI_data_table.province_capital[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. province_capital_1.."%    "  .. gr_c 
                         elseif UI_data_table.province_capital[1] <= negative_value then
...
-- class 2              
                local province_capital_2 = UIFormatValuesBehindComma(UI_data_table.province_capital[2]) 
                        if UI_data_table.province_capital[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. province_capital_2.."%"  .. gr_c.. line
                         elseif UI_data_table.province_capital[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. province_capital_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. province_capital_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local province_capital_3 = UIFormatValuesBehindComma(UI_data_table.province_capital[3]) 
                        if UI_data_table.province_capital[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. province_capital_3.."%     " ..gr_c 
                         elseif UI_data_table.province_capital[3] <= negative_value then
                          text = text.."III:   ".. r_o .. province_capital_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. province_capital_3.."%     "  ..y_c
                        end
-- class 4              
                local province_capital_4 = UIFormatValuesBehindComma(UI_data_table.province_capital[4]) 
                        if UI_data_table.province_capital[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. province_capital_4.."%"  .. gr_c .. line
                         elseif UI_data_table.province_capital[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. province_capital_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. province_capital_4.."%"  ..y_c .. line
                        end
                end
-- set public order next
-- Header, set header first then get all class values
                if UI_data_table.total_public_order > min_value or UI_data_table.total_public_order <= negative_value then
                text = text.. UI_data_table.public_order_tx..line
--Block
-- start line 1 class 1
                local public_order_1 = UIFormatValuesBehindComma(UI_data_table.public_order[1]) 
                        if UI_data_table.public_order[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. public_order_1.."%    "  .. gr_c 
                         elseif UI_data_table.public_order[1] <= negative_value then
...
-- class 2              
                local public_order_2 = UIFormatValuesBehindComma(UI_data_table.public_order[2]) 
                        if UI_data_table.public_order[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. public_order_2.."%"  .. gr_c.. line
                         elseif UI_data_table.public_order[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. public_order_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. public_order_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local public_order_3 = UIFormatValuesBehindComma(UI_data_table.public_order[3]) 
                        if UI_data_table.public_order[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. public_order_3.."%     " ..gr_c 
                         elseif UI_data_table.public_order[3] <= negative_value then
...
-- class 4              
                local public_order_4 = UIFormatValuesBehindComma(UI_data_table.public_order[4]) 
                        if UI_data_table.public_order[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. public_order_4.."%"  .. gr_c .. line
                         elseif UI_data_table.public_order[4] <= negative_value then
...
---- set food next
-- Header, set header first then get all class values
                if UI_data_table.total_food > min_value or UI_data_table.total_food <= negative_value then
                text = text.. UI_data_table.food_tx..line
--Block
-- start line 1 class 1
                local food_1 = UIFormatValuesBehindComma(UI_data_table.food[1]) 
                        if UI_data_table.food[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. food_1.."%    "  .. gr_c 
                         elseif UI_data_table.food[1] <= negative_value then
                          text = text.."I:     ".. r_o .. food_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. food_1.."%    "  ..y_c
                        end
-- class 2              
                local food_2 = UIFormatValuesBehindComma(UI_data_table.food[2]) 
                        if UI_data_table.food[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. food_2.."%"  .. gr_c.. line
                         elseif UI_data_table.food[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. food_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. food_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local food_3 = UIFormatValuesBehindComma(UI_data_table.food[3]) 
                        if UI_data_table.food[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. food_3.."%     " ..gr_c 
                         elseif UI_data_table.food[3] <= negative_value then
                          text = text.."III:   ".. r_o .. food_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. food_3.."%     "  ..y_c
                        end
-- class 4              
                local food_4 = UIFormatValuesBehindComma(UI_data_table.food[4]) 
                        if UI_data_table.food[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. food_4.."%"  .. gr_c .. line
                         elseif UI_data_table.food[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. food_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. food_4.."%"  ..y_c .. line
                        end
                end
-- set taxation next
-- Header, set header first then get all class values
                if UI_data_table.total_taxation > min_value or UI_data_table.total_taxation <= negative_value then
                text = text.. UI_data_table.taxation_tx..line
--Block
-- start line 1 class 1
                local taxation_1 = UIFormatValuesBehindComma(UI_data_table.taxation[1]) 
                        if UI_data_table.taxation[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. taxation_1.."%    "  .. gr_c 
                         elseif UI_data_table.taxation[1] <= negative_value then
                          text = text.."I:     ".. r_o .. taxation_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. taxation_1.."%    "  ..y_c
                        end
-- class 2              
                local taxation_2 = UIFormatValuesBehindComma(UI_data_table.taxation[2]) 
                        if UI_data_table.taxation[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. taxation_2.."%"  .. gr_c.. line
                         elseif UI_data_table.taxation[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. taxation_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. taxation_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local taxation_3 = UIFormatValuesBehindComma(UI_data_table.taxation[3]) 
                        if UI_data_table.taxation[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. taxation_3.."%     " ..gr_c 
                         elseif UI_data_table.taxation[3] <= negative_value then
                          text = text.."III:   ".. r_o .. taxation_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. taxation_3.."%     "  ..y_c
                        end
-- class 4              
                local taxation_4 = UIFormatValuesBehindComma(UI_data_table.taxation[4]) 
                        if UI_data_table.taxation[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. taxation_4.."%"  .. gr_c .. line
                         elseif UI_data_table.taxation[4] <= negative_value then
...
-- Header, set header first then get all class values
                if UI_data_table.total_culture_bonuses > min_value or UI_data_table.total_culture_bonuses <= negative_value then
...
-- start line 1 class 1
                local culture_bonuses_1 = UIFormatValuesBehindComma(UI_data_table.culture_bonuses[1]) 
                        if UI_data_table.culture_bonuses[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. culture_bonuses_1.."%    "  .. gr_c 
                         elseif UI_data_table.culture_bonuses[1] <= negative_value then
                          text = text.."I:     ".. r_o .. culture_bonuses_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. culture_bonuses_1.."%    "  ..y_c
                        end
-- class 2              
                local culture_bonuses_2 = UIFormatValuesBehindComma(UI_data_table.culture_bonuses[2]) 
                        if UI_data_table.culture_bonuses[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. culture_bonuses_2.."%"  .. gr_c.. line
                         elseif UI_data_table.culture_bonuses[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. culture_bonuses_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. culture_bonuses_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local culture_bonuses_3 = UIFormatValuesBehindComma(UI_data_table.culture_bonuses[3]) 
                        if UI_data_table.culture_bonuses[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. culture_bonuses_3.."%     " ..gr_c 
                         elseif UI_data_table.culture_bonuses[3] <= negative_value then
...
-- class 4              
                local culture_bonuses_4 = UIFormatValuesBehindComma(UI_data_table.culture_bonuses[4]) 
                        if UI_data_table.culture_bonuses[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. culture_bonuses_4.."%"  .. gr_c .. line
                         elseif UI_data_table.culture_bonuses[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. culture_bonuses_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. culture_bonuses_4.."%"  ..y_c .. line
                        end
                end
-- set majority_religion next
-- Header, set header first then get all class values
                if UI_data_table.total_majority_religion > min_value or UI_data_table.total_majority_religion <= negative_value then
...
-- start line 1 class 1
                local majority_religion_1 = UIFormatValuesBehindComma(UI_data_table.majority_religion[1]) 
                        if UI_data_table.majority_religion[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. majority_religion_1.."%    "  .. gr_c 
                         elseif UI_data_table.majority_religion[1] <= negative_value then
                          text = text.."I:     ".. r_o .. majority_religion_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. majority_religion_1.."%    "  ..y_c
                        end
-- class 2              
                local majority_religion_2 = UIFormatValuesBehindComma(UI_data_table.majority_religion[2]) 
                        if UI_data_table.majority_religion[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. majority_religion_2.."%"  .. gr_c.. line
                         elseif UI_data_table.majority_religion[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. majority_religion_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. majority_religion_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local majority_religion_3 = UIFormatValuesBehindComma(UI_data_table.majority_religion[3]) 
                        if UI_data_table.majority_religion[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. majority_religion_3.."%     " ..gr_c 
                         elseif UI_data_table.majority_religion[3] <= negative_value then
...
-- class 4              
                local majority_religion_4 = UIFormatValuesBehindComma(UI_data_table.majority_religion[4]) 
                        if UI_data_table.majority_religion[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. majority_religion_4.."%"  .. gr_c .. line
                         elseif UI_data_table.majority_religion[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. majority_religion_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. majority_religion_4.."%"  ..y_c .. line
                        end
                end   
-- set buidlings next
-- Header, set header first then get all class values
                if UI_data_table.total_buidlings > min_value or UI_data_table.total_buidlings <= negative_value then
                text = text.. UI_data_table.buidlings_tx..line
--Block
-- start line 1 class 1
                local buidlings_1 = UIFormatValuesBehindComma(UI_data_table.buidlings[1]) 
                        if UI_data_table.buidlings[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. buidlings_1.."%    "  .. gr_c 
                         elseif UI_data_table.buidlings[1] <= negative_value then
                          text = text.."I:     ".. r_o .. buidlings_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. buidlings_1.."%    "  ..y_c
                        end
-- class 2              
                local buidlings_2 = UIFormatValuesBehindComma(UI_data_table.buidlings[2]) 
                        if UI_data_table.buidlings[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. buidlings_2.."%"  .. gr_c.. line
                         elseif UI_data_table.buidlings[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. buidlings_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. buidlings_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local buidlings_3 = UIFormatValuesBehindComma(UI_data_table.buidlings[3]) 
                        if UI_data_table.buidlings[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. buidlings_3.."%     " ..gr_c 
                         elseif UI_data_table.buidlings[3] <= negative_value then
                          text = text.."III:   ".. r_o .. buidlings_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. buidlings_3.."%     "  ..y_c
                        end
-- class 4              
                local buidlings_4 = UIFormatValuesBehindComma(UI_data_table.buidlings[4]) 
                        if UI_data_table.buidlings[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. buidlings_4.."%"  .. gr_c .. line
                         elseif UI_data_table.buidlings[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. buidlings_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. buidlings_4.."%"  ..y_c .. line
                        end
                end
-- set under_siege next
-- Header, set header first then get all class values
                if UI_data_table.total_under_siege > min_value or UI_data_table.total_under_siege <= negative_value then
                text = text.. UI_data_table.under_siege_tx..line
--Block
-- start line 1 class 1
                local under_siege_1 = UIFormatValuesBehindComma(UI_data_table.under_siege[1]) 
                        if UI_data_table.under_siege[1] > min_value then  
                          text = text.."I:     ".. gr_o .."+".. under_siege_1.."%    "  .. gr_c 
                         elseif UI_data_table.under_siege[1] <= negative_value then
                          text = text.."I:     ".. r_o .. under_siege_1.."%    "  ..r_c 
                         else text = text.."I:     ".. y_o .. under_siege_1.."%    "  ..y_c
                        end
-- class 2              
                local under_siege_2 = UIFormatValuesBehindComma(UI_data_table.under_siege[2]) 
                        if UI_data_table.under_siege[2] > min_value then 
                          text = text.."    II:   ".. gr_o .."+".. under_siege_2.."%"  .. gr_c.. line
                         elseif UI_data_table.under_siege[2] <= negative_value then
                          text = text.."    II:   ".. r_o .. under_siege_2.."%"  ..r_c.. line
                         else text = text.."    II:   ".. y_o .. under_siege_2.."%"  ..y_c.. line
                        end
-- next line
-- class 3              
                local under_siege_3 = UIFormatValuesBehindComma(UI_data_table.under_siege[3]) 
                        if UI_data_table.under_siege[3] > min_value then 
                          text = text.."III:   ".. gr_o .."+".. under_siege_3.."%     " ..gr_c 
                         elseif UI_data_table.under_siege[3] <= negative_value then
                          text = text.."III:   ".. r_o .. under_siege_3.."%     "  ..r_c 
                         else text = text.."III:   ".. y_o .. under_siege_3.."%     "  ..y_c
                        end
-- class 4              
                local under_siege_4 = UIFormatValuesBehindComma(UI_data_table.under_siege[4]) 
                        if UI_data_table.under_siege[4] > min_value then 
                          text = text.."  IV:   ".. gr_o .."+".. under_siege_4.."%"  .. gr_c .. line
                         elseif UI_data_table.under_siege[4] <= negative_value then
                          text = text.."  IV:   ".. r_o .. under_siege_4.."%"  ..r_c .. line
                         else text = text.."  IV:   ".. y_o .. under_siege_4.."%"  ..y_c .. line
                        end
                end
---- set tech
--local technology = UIFormatValuesBehindComma(UI_data_table.technology) 
        --if UI_data_table.technology > min_value then
                --text = text.. UI_data_table.technology_tx.. gr_o .."+".. technology .."%"  .. gr_c .. line
        --elseif UI_data_table.technology <= negative_value then
                --text = text.. UI_data_table.technology_tx.. r_o .. technology .."%" ..r_c .. line
        --end
        local provinceName = UIProvinceFromRegionname(UI_data_table.curr_region_name)
...
-- Contains all adjustments of a regions population and requires the region script interface to passed to it
function UIRegionPopGrowth()PopLog("Start UIRegionPopGrowth")
        local region = UI_data_table.curr_region
        local regionName = region:name()
        local rp = (UIPopulation and UIPopulation[regionName]) or (region_table and region_table[regionName])
        if rp then
                ...
                --2)Food Shortage
              PopLog("foodShortage UIRegionPopGrowth")
                local foodShortage = false
                if faction:has_food_shortage() then
                        foodShortage = true
                end
                --3)Majority Religion & technology
              PopLog("majorityCulture UIRegionPopGrowth")
                ...
                --7)Faction Capitol
                local regionIsHome = false
                if region == homeRegion then    
                        regionIsHome = true
                end
                PopLog("provCap UIRegionPopGrowth")
                --8) Province Capitol
                local provCap = false
                if region:building_superchain_exists("rom_SettlementMajor") then
                        provCap = true
                end
                ...
                PopLog("Majority Religion & technology for class: "..i)
                        if majorityCulture then
                                ...
                                for k, v in pairs(tech_pop_growth_own_culture_table) do
                                        if faction:has_technology(k) then
                                                UI_technology[i] = tech_pop_growth_own_culture_table[k][i]
                                                UI_data_table.technology = UI_data_table.technology + UI_technology[i]
                                        end                                     
                                end
                  ...
                  if regionPopTable[i] < population_modifier["base_pop_growth_min_size"][i] then
                    UI_minimum_growth_per_class[i] = UI_minimum_growth_per_class[i] + population_modifier["base_pop_growth"][i] -- no % only integer
                       ...
                       -- set food
                        ...
                        -- set under_siege
                        PopLog("UI_data_table.food") 
                        UI_data_table.under_siege = {UI_buildings[1], UI_buildings[2], UI_buildings[3], UI_buildings[4]}
                        UI_data_table.total_under_siege = (UI_Under_Siege[1] + UI_Under_Siege[2] + UI_Under_Siege[3] + UI_Under_Siege[4]) /4
                        -- technology buffs
                       -- UI_data_table.technology = UI_data_table.technology / 4
                        UI_data_table.total_noble_growth =  UIbase_growth[1] + UI_public_order[1] + UI_food[1] + UI_taxation[1] + UI_Faction_Capital[1] + UI_Province_Capital[1] + UI_Culture[1] + UI_majority_religion[1] + UI_buildings[1] + UI_Under_Siege[1]
                        ...
                        UI_data_table.total_citizen_growth =  UIbase_growth[2] + UI_public_order[2] + UI_food[2] + UI_taxation[2] + UI_Faction_Capital[2] + UI_Province_Capital[2] + UI_Culture[2] + UI_majority_religion[2] + UI_buildings[2] + UI_Under_Siege[2]
                        ...
                        UI_data_table.total_poor_growth =   UIbase_growth[3] + UI_public_order[3] + UI_food[3] + UI_taxation[3] + UI_Faction_Capital[3] + UI_Province_Capital[3] + UI_Culture[3] + UI_majority_religion[3] + UI_buildings[3] + UI_Under_Siege[3]
                        ...
                        UI_data_table.total_foreign_growth = UIbase_growth[4] + UI_public_order[4] + UI_food[4] + UI_taxation[4] + UI_Faction_Capital[4] + UI_Province_Capital[4] + UI_Culture[4] + UI_majority_religion[4] + UI_buildings[4] + UI_Under_Siege[4] 
...
local region_name = UI_data_table.curr_region_name
        for popClass  = 1, 4 do
                if popClass == 1 then UI_data_table.immigration_1 = region_desire[region_name][popClass + 4] end
                if popClass == 2 then UI_data_table.immigration_2 = region_desire[region_name][popClass + 4] end
                if popClass == 3 then UI_data_table.immigration_3 = region_desire[region_name][popClass + 4] end
                if popClass == 4 then UI_data_table.immigration_4 = region_desire[region_name][popClass + 4] end
        end
end

-- UI shaddow calc for econmic effects
...
--      PopLog("RegionBundle category not default", "ApplyRegionBundle()")
                ...
                -- 1) Foreigner %
                -- heartland
                if (foreignTotalPercent < 0.1) then
                ...
                --Provincial
                elseif (foreignTotalPercent < 0.5) then
                ...
                --Colony
                elseif (foreignTotalPercent < 0.75) then
...
Debug("UIDevPoinsIconTooltip")
                 local function rint(x) if x>=0 then return math.floor(x+0.5) else return -math.floor(-x+0.5) end end
                 ...
                 local r_o = UI_data_table.col_red_open
                 local r_c = UI_data_table.col_red_close
                 local immigration_1 = rint(UI_data_table.predicted_noble_growth_number)
                 local immigration_2 = rint(UI_data_table.predicted_citizen_growth_number)
                 local immigration_3 = rint(UI_data_table.predicted_poor_growth_number)
                 local immigration_4 = rint(UI_data_table.predicted_foreign_growth_number)
                 local tx_im_1 = ""
                 local tx_im_2 = ""
                 local tx_im_3 = ""
                 local tx_im_4 = ""
...
Debug("UIDevPoinsIconTooltip: Local Vars set successfully")
                --PopLog("Start: change tooltip +/- for immigration", "UIDevPoinsIconTooltip()")
                        ...
                        if immigration_1 > 0 then
                                tx_im_1 = gr_o.."(+"..immigration_1..")"..gr_c
                        elseif immigration_1 < 0 then
                        ...
                        if immigration_2 > 0 then
                                tx_im_2 = gr_o.."(+"..immigration_2..")"..gr_c
                        elseif immigration_2 < 0 then
                        ...
                        if immigration_3 > 0 then
                                 tx_im_3 = gr_o.."(+"..immigration_3..")"..gr_c
                        elseif immigration_3 < 0 then
                        ...
                        if immigration_4 > 0 then
                                 tx_im_4 = gr_o.."(+"..immigration_4..")"..gr_c
                        elseif immigration_4 < 0 then
...
Debug("UIDevPoinsIconTooltip: immigration set successfully")
                        if UI_data_table.PoR2_region_noble > 0 then
                        ...
                        elseif UI_data_table.PoR2_region_noble < 0 then
...
Debug("UIDevPoinsIconTooltip: PoR2_region_noble")
                        if UI_data_table.PoR2_region_middle > 0 then
                        ...
                        elseif UI_data_table.PoR2_region_middle < 0 then
...
Debug("UIDevPoinsIconTooltip: PoR2_region_middle")
                        if UI_data_table.PoR2_region_foreign > 0 then
                        ...
                        elseif UI_data_table.PoR2_region_foreign < 0 then
...
Debug("UIDevPoinsIconTooltip: PoR2_region_foreign")
                        if UI_data_table.PoR2_region_total > 0 then
                        ...
                        elseif UI_data_table.PoR2_region_total < 0 then
...
-- iterate ALL regions in the world (any owner)
    local world = scripting.game_interface:model():world()
    local all_regions = world:region_manager():region_list()
    for i = 0, all_regions:num_items() - 1 do
        local r = all_regions:item_at(i)
        local regionName_2 = r:name()
        ...
        if province_name == province then
            local rp = (UIPopulation and UIPopulation[regionName_2]) or (region_table and region_table[regionName_2])
            if rp then
                noble_pop    = noble_pop    + (rp[1] or 0)
                citizens_pop = citizens_pop + (rp[2] or 0)
                poor_pop     = poor_pop     + (rp[3] or 0)
                foreign_pop  = foreign_pop  + (rp[4] or 0)
            end
        end
    end
...
PopLog("UIRetrievePopulation")
        ...
        scripting.game_interface:add_time_trigger("Change_Grwoth_Bar", 0.3)  
...
local function UIChangeOnButtonPressed(context)  
  if context.string == "button_show_province" then
    scripting.game_interface:add_time_trigger("Change_Pop", 0.1);
    return;
  end;

-- growth number changes on button activate slot, so we have to take click into account
  if context.string == "button_activate_slot" then
    for i = 0.1, 1, 0.1 do scripting.game_interface:add_time_trigger("Change_Pop", i);
      end;
    return;
  end;

  if (context.string == "button_recruitment") then
    scripting.game_interface:add_time_trigger("hide_panel", 0.2);
  end;

  if context.string == "button_province_details" then
  ...
  elseif (context.string == "button_cycle_right") then
    scripting.game_interface:add_time_trigger("Change_Details", 0.2);

  elseif (context.string == "button_cycle_left") then
    scripting.game_interface:add_time_trigger("Change_Details", 0.2);

  elseif (context.string == "checkbox_tax_exempt") then
    scripting.game_interface:add_time_trigger("Change_Details", 0.2);
    return;
  end;
  ...
  local endstring = UI_data_table.cancel_building_string
    if string.find(CancelBuilding, endstring) then-- tooltip string for cancel buildings
      for i = 0.1, 1, 0.1 do scripting.game_interface:add_time_trigger("Change_Pop", i);
      end;
    end;
end;
        ...
        Log("Panel Opened " ..context.string)
         if context.string == "settlement_panel" then
          -- calling it here leads to an issue as the region key might not be valid anymore and we retrieve it on region select
                 scripting.game_interface:add_time_trigger("Change_Pop", 0.1)
         -- calling it here leads to an issue as the region key might not be valid anymore and we retrieve it on region select
                 scripting.game_interface:add_time_trigger("Change_Grwoth_Bar", 1)
         end
...
local growth_bar_uic = UIComponent(growth_icon_uic:Find("growth_bar"))
...
growth_bar_uic:SetMoveable(true)
  ...
  local provinceName = UIProvinceFromRegionname(UI_data_table.curr_region_name)
    ...
    Debug("Province Name Number of Regions = 2")
     ...
     if save_X_2 == 0 
      ...
      elseif save_X_3 ~= 0 then original_X_2 = growth_bar_pX - 70
      elseif save_X_4 ~= 0 then original_X_2 = growth_bar_pX - 70 end
      ...
      elseif save_X_2 == 0 
       and save_X_3 ~= 0 
        then save_X_2 = original_X_2 + 330 
    ...
    if province_name_table[provinceName][3] == 3 then
      Debug("Province Name Number of Regions = 3")
     local growth_bar_pX, growth_bar_pY = growth_bar_uic:Position()
     if save_X_2 == 0 
      and save_X_3 == 0 
       and save_X_4 == 0 
      then original_X_3 = growth_bar_pX 

      elseif save_X_2 ~= 0 then original_X_3 = growth_bar_pX - 330
      elseif save_X_4 ~= 0 then original_X_3 = growth_bar_pX - 70 end
    ...
    growth_bar_uic:MoveTo(save_X_3, growth_bar_pY)
  end
    ...
    if province_name_table[provinceName][3] == 4 then
     ...
     if save_X_2 == 0 
      ...
      elseif save_X_2 ~= 0 then original_X_4 = growth_bar_pX - 330
      elseif save_X_3 ~= 0 then original_X_4 = growth_bar_pX - 70 end
    ...
    growth_bar_uic:MoveTo(save_X_4, growth_bar_pY) end
...
local function UIChangeCampaignComponentsOnMouseOn(context)
        if context.string == "dev_points icon" 
                then UIGetImmigration()
                        --PopLog("End:", "UIGetImmigration()")
                        UIDevPoinsIconTooltip(context.component)
        end

        if context.string == "province_dev_points" then 
                UIGrowthTooltip(context.component)      
          end

        if context.string == "icon_command" then 
        local text = UIComponent(context.component):GetTooltipText()
        local string = UI_data_table.authoritarian_string
                if string.find(text, "Security") or string.find(text, "First Class") then
                        UIComponent(context.component):SetTooltipText("Your "..UI_data_table.col_yellow_open ..UI_data_table.noble_pop_name..UI_data_table.col_yellow_close.. " population in the province.")--" Province and faction capitals can increase regional "..UI_data_table.col_yellow_open ..UI_data_table.noble_pop_name..UI_data_table.col_yellow_close.." growth rates")
                end
        end

        if context.string == "dy_command" then
        local text = UIComponent(context.component):GetTooltipText()
        ...
        if context.string == "icon_subterfuge"  then
        local text = UIComponent(context.component):GetTooltipText()
        local string = UI_data_table.cunning_string
                if string.find(text, "Security") or string.find(text, "Second and Third Class") then
        ...
        if context.string == "dy_subterfuge"  then
        local text = UIComponent(context.component):GetTooltipText()
        ...
        if context.string == "icon_zeal" then 
        local text = UIComponent(context.component):GetTooltipText()
        local string = UI_data_table.zealous_string
                if string.find(text, "Security") or string.find(text, "Foreign Class") then
        ...
        if context.string == "dy_zeal" then
        local text = UIComponent(context.component):GetTooltipText()
        local string = UI_data_table.authoritarian_string
                if string.find(text, "Security") or string.find(text, "Foreign Class") then
                        UIComponent(context.component):SetTooltipText("Your "..UI_data_table.col_yellow_open ..UI_data_table.foreign_pop_name..UI_data_table.col_yellow_close.. " population in the province.")--..UI_data_table.col_yellow_open ..UI_data_table.foreign_pop_name..UI_data_table.col_yellow_close.." growth rates are heavily influenced by migration")
        ...
        if context.string == "dy_prosperity" then
                local y_o = UI_data_table.col_yellow_open
                local y_c = UI_data_table.col_yellow_close
                ...
                if context.string == "tx_prosperity" then
...
function RecListenerCheckNavy(force)
        if force:is_navy()then return true
                else return false
        end
end
...
if current_character == nil then-- PopLog("Character is Gone - Prevent Crash", "setClassPop()") 
  return end 

        if RecListenerCheckNavy(current_character:military_force()) then
...
function UIManpowerCostTooltip(context)
        ...
        if recruitableUnitCount == nil then return end
        local recruitableUnitList = {} -- add them into a list
        setClassPop()

        for i = 0, recruitableUnitCount - 1 do
            local recruitableUnit = UIComponent(list_box:Find(i)):Id()
             if string.find(recruitableUnit, "_recruitable") then
                table.insert(recruitableUnitList, recruitableUnit)
             end
        end

        for key, recruitableUnit in ipairs(recruitableUnitList) do
          ...
          Log("Start")
            ...
            if classKey == 0 then UIComponent(UnitCard_uic):SetTooltipText("Error Unit has no class key assigned!") 
          elseif classKey > 0 then
            ...
            local RegionDisplay = RegionKeytoRegionLoc[UI_data_table.curr_region_name]
            ...
            text = string.gsub(text, endString, "")
                ...
                if ManpowerAvailable >= ManpowerCosts then
...
if (context.string == "hide_panel") then
        UIManpowerCostTooltip(context)        
  end 

if (context.string == "Change_Grwoth_Bar") then
        ChangeGrowthBar(context)        
  end 

if (context.string == "hide_panel_mercs") then
        Mercs_Levies_UIManpowerCosts(context)   
  end
        
if (context.string == "units_created") then
        Merc_Levy_RecListenerUnitsCreated(context)      
  end

if (context.string == "units_disbanded") then
        RecListenerDisbandUnits(context)        
 end

if (context.string == "Change_Details") then
        UIChangeProvinceDetailsOnTimeTrigger(context)
  end

  if (context.string == "Move_Icons") then
      UIMoveProvinceIconsOnTimeTrigger(context)
  end;

if (context.string == "Change_Pop") then
        UIChangeOnTimeTrigger(context)
  end

if (context.string == "Show_Total_Pop") then
...
local function OnShortcutTriggered(context)
        if context.string == "show_recruitment_units" then
        ...
        if context.string == "show_recruitment_agents" then
                local regionName = current_character:region():name()
                        for i =1, 4 do
                                ui_merc_levy_region_table[i] = UIPopulation[regionName][i]
                        end
...
if faction:is_human() then -- maybe time trigger ?
               ...
               Debug("CQI:"..curr_cqi)
                if disband_unit_table[tostring(curr_cqi)] then
...
local cqi = char:cqi()
        local regionName = ""
        if current_character:military_force():is_navy() then 
          local x, y = current_character:logical_position_x(), current_character:logical_position_y()
          regionName = getClosestPortRegion(x, y, current_character:faction()) 
         else regionName = current_character:region():name() end 
        ...
        for i = 0, force:unit_list():num_items() - 1 do
                ...
                for _, v in pairs(disband_unit_table[tostring(cqi)]) do
                if unit_key == v[1] and unit_strenght == v[2] then
                        disband_unit_table[tostring(cqi)][_] = {nil, nil, nil}
                      --  PopLog("Unit Key remove from table ".. _ .."  "..unit_key)
                      --  PopLog("Unit Size " .. tostring(unit_strenght))
                  break 
                end     
                end
        end

        for i,value in pairs(disband_unit_table[tostring(cqi)]) do              
                if value[1] ~= nil then
                        local unit_name = value[1]
                        local class = UIRetrieveClass(unit_name, factionName)
                        for v,mercenary in pairs (mercenary_units_table) do 
                                if mercenary == unit_name 
                                        then class = Mercs_Levies_GetClass(unit_name, char)
                                   break
                                end
                        end
                        local costs = RecListenerCheckUnitPercentageCosts(unit_name, class, value[2])
                        --PopLog("Add pop for cqi: "..cqi.. " : ".. unit_name .. "  Class:  " .. class .. " Costs: " .. costs)
                        UIPopulation[regionName][class] = UIPopulation[regionName][class] + costs
                end
          end
          disband_unit_table = {} 
...
if not ArmyExists then
        for i,v in pairs(disband_unit_table) do
                if disband_unit_table[tostring(i)] then
                 for k, value in pairs(disband_unit_table[tostring(i)]) do
                        local unit_name = value[1]
                        local class = UIRetrieveClass(unit_name, factionName)
                        ...
                        -- end
                        local costs = RecListenerCheckUnitPercentageCosts(unit_name, class, value[2])
                       -- PopLog("Add pop for: ".. unit_name .. "  Class:  " .. class .. " Costs: " .. costs.. "for region: "..value[3])
...
local isRecruitedUnit = string.find(component, "_recruitable")
        if isRecruitedUnit and UIComponent(context.component):CurrentState() == "active" 
                then unitKey = string.gsub(component, "_recruitable", "")
                local factionName = UIGetLocalFactionName()
                local regionName = ""
                if current_character:military_force():is_navy() then 
                  local x, y = current_character:logical_position_x(), current_character:logical_position_y()
                  regionName = getClosestPortRegion(x, y, current_character:faction()) 
                else regionName = current_character:region():name() end; -- make sure navies remove pop from capital
                local class = UIRetrieveClass(unitKey, factionName)
                Log("class: "..class)
                local costs = RecListenerRetrieveunitcosts(unitKey, class)
                Log("costs: "..costs)
                ...
                if not recruitmentOrders[tostring(current_character:cqi())] then Log("New table recruitmentOrders: "..current_character:cqi()) recruitmentOrders[tostring(current_character:cqi())] = {} end
...
local armylist = context:faction():military_force_list()
        for i = 0, armylist:num_items() -1 do
          ...
          local army_cqi = army:general_character():cqi()  
          Debug("CQI: "..army_cqi)
          if army:is_army() or army:is_navy() and recruitmentOrders[tostring(army_cqi)] and army_size_cqi[tostring(army_cqi)] then
           local x = army:general_character():logical_position_x()
           local y = army:general_character():logical_position_y()

          Debug("CQI is saved in recruitmentOrders : "..army_cqi)
          local char_army_size = army:unit_list():num_items()                 
            if x..y ~= army_size_cqi[tostring(army_cqi)][1] then
              Debug("Position changed remove recruitmentOrders: "..army_cqi)
              recruitmentOrders[tostring(army_cqi)] = nil 
            end;

               if char_army_size == army_size_cqi[tostring(army_cqi)][2] then
                Debug("Army size is the same char_army_size: "..char_army_size)

               elseif char_army_size > army_size_cqi[tostring(army_cqi)][2] then
               ...
               Debug("unitQueueLength: "..tostring(unitQueueLength))
                for j = 1, math.min(deleteQueueLength, unitQueueLength) do
...
local function RecListenerAddUnitTable(context)
    if (context.string == "button_disband") then
     --   Log("Button Disband hit")
        ...
        if current_character:military_force():is_navy() then 
          local x, y = current_character:logical_position_x(), current_character:logical_position_y()
          regionName = getClosestPortRegion(x, y, current_character:faction()) else regionName = current_character:region():name() end 
                for i = 0, force:unit_list():num_items() - 1 do
                        local unit = force:unit_list():item_at(i)
                        local unit_key = unit:unit_key()
                        local unit_strenght = unit:percentage_proportion_of_full_strength()
                        if not disband_unit_table[tostring(cqi)] then disband_unit_table[tostring(cqi)] = {} end
...
local factionName = UIGetLocalFactionName()
    ...
    if RecListenerCheckNavy(current_character:military_force()) then
      local x, y = current_character:logical_position_x(), current_character:logical_position_y()
      regionName = getClosestPortRegion(x, y, current_character:faction()) 
     else regionName = current_character:region():name() 
    end; 
...
function Mercs_Levies_LocalClass(faction)
        if mercenary_faction_class_table[faction]
                then return mercenary_faction_class_table[faction]
        else
...
local class = 4 
local class_key = Mercs_Levies_LocalClass(curr_char:region():garrison_residence():faction():name())

    for i, unit in pairs (mercenary_units_to_faction_class[class_key]) do
        if unit == unit_key  
            then class = UIRetrieveClass(unit_key, curr_char:faction():name())
           return class
        end
    end
  return class
...
for i = 0, recruitableUnitCount - 1 do
    local recruitableUnit = UIComponent(list_box:Find(i)):Id()
        if string.find(recruitableUnit, "_mercenary") then
        ...
        if string.find(text, UI_data_table.cannot_recruit_string) then 
        ...
        if string.find(text, UI_data_table.hire_mercs_string)
            then unit_card:SetOpacity(250)
        end      

          if ManpowerAvailable >= unit_costs then 
...
if TableisEmpty(Merc_Levy_recruitmentOrders) then return end
if TableisEmpty(char_merc_levy_units) then return end

local new_merc_levy_units = {}  
if current_character:military_force():unit_list():num_items() > 1
 then --PopLog("Create unit table for NEW mercs")
        local regionName = current_character:region():name()
        local force = current_character:military_force()
        for i = 0, force:unit_list():num_items() - 1 do
                local unit = force:unit_list():item_at(i)
...
for i = 0, current_character:military_force():unit_list():num_items() - 1 do 
    local key = new_merc_levy_units[i] 
    --  PopLog("Check Unit key:  "..key)
        for x = 0, current_character:military_force():unit_list():num_items() - 1 do 
                        if key == char_merc_levy_units[x] then
...
--PopLog("Start check: Remaining units")
        for i,value in pairs(new_merc_levy_units) do            
                if value ~= nil then
...
local function Merc_Levy_RecListenerAddUnitTable(context)
        ...
        if (context.string == "button_mercenaries") then
                local regionName = current_character:region():name()
                        for i =1, 4 do
                                ui_merc_levy_region_table[i] = UIPopulation[regionName][i]
        ...
        if (context.string == "button_setrapy") then
                local regionName = current_character:region():name()
                        for i =1, 4 do
                                ui_merc_levy_region_table[i] = UIPopulation[regionName][i]
        ...
        local isRecruitedMerc = string.find(context.string, "_mercenary")
                ...
                if isRecruitedMerc and UIComponent(context.component):CurrentState() == "active" 
                        then Merc_Levy_AddUnitTable()
                                ...
                                if not Merc_Levy_recruitmentOrders[tostring(current_character:cqi())] then Merc_Levy_recruitmentOrders[tostring(current_character:cqi())] = {} end
...
local function Merc_Levy_RecListenerAcceptHire(context)
        local component = UIComponent(context.component):Id()
        if current_character ~= nil and component == "button_confirm" then 
...
local curr_char = context:character()
  ...
  if not curr_char:faction():is_human() then
    return
  end

  if curr_char:is_polititian() then
    return
  end
...
-- fleets don't have a region so we always use the capital instead
      local region = ""
      if curr_char:has_region() then region = curr_char:region() end
      
-- skip further checks if army has no region/sea region and is not in port     
      if not curr_char:has_region() and curr_char:military_force():is_army() and not curr_char:has_garrison_residence()
        then scripting.game_interface:apply_effect_bundle_to_characters_force("PoR2_disable_replenishment_region_wide", cqi,-1)
      end

      if not curr_char:has_region() then region = curr_char:faction():home_region() end
      local regionName = region:name()
...
-- we save the amount of units needed to completly refill the unit
             unitStrenght = unit_to_pop_table[unitKey][2] - unitStrenght
                ...
                if not ReplenishmentArmyTable[unitKey] then 
        ...
        for unitKey, tab in pairs(ReplenishmentArmyTable) do
         ...
         local class = UIRetrieveClass(unitKey, curr_char:faction():name())
            for v,mercenary in pairs (mercenary_units_table) do 
                if mercenary == unitKey 
                 then class = Mercs_Levies_GetClass(unitKey, curr_char)
                end
            end
...
-- if class manpower in the region is not high enough we block replenishment
      if curr_char:has_region()
       and culture ~= region:owning_faction():state_religion() then
      ...
      return end
            ...
            if curr_char:has_region()
             and culture == region:owning_faction():state_religion()
              then if ReplenishmentPopTable[class] < math.ceil(costs*0.3) then 
              Log("Manpower in Region: "..regionName.. " too low for: " .. costs.." Block all replenishment! for: "..cqi)
              scripting.game_interface:apply_effect_bundle_to_characters_force("PoR2_disable_replenishment_region_wide", cqi,-1)
              return
              else ReplenishmentPopTable[class] = ReplenishmentPopTable[class] - math.ceil(costs*0.3)
               end
            end
...
if context:character():faction():name() == "rebels"
 ...
 Log("context:character():military_force():is_army()")
  ...
  if context:character():has_region() 
    then region = context:character():region():name() else return end
   ...
   if not rebel_cqi[tostring(cqi)] then rebel_cqi[tostring(cqi)] = {}
    for i = 0, force:unit_list():num_items() - 1 do
      if i < 3 then 
       local unit = force:unit_list():item_at(i)
       local unit_key = unit:unit_key()
   ...
   elseif rebel_cqi[tostring(cqi)] then 
     for i = 0, force:unit_list():num_items() - 1 do
       local unit = force:unit_list():item_at(i)
       local unit_key = unit:unit_key()
       rebel_cqi[tostring(cqi)][i] = unit_key
    ...
    local force = context:character():military_force()
     ...
     for i = 0, force:unit_list():num_items() - 1 do
      ...
      local unit_key = unit:unit_key()
        ...
        for i = 0, context:character():military_force():unit_list():num_items() - 1 do 
          local key = new_rebel_units[i] 
            for x = 0, context:character():military_force():unit_list():num_items() - 1 do
              if key == rebel_cqi[tostring(cqi)][x] then
        ...
        for i,value in pairs(new_rebel_units) do                
           if value ~= nil then
             local unit_key = value
...
if force:is_navy() then return end 
...
local force = character:military_force()
 ...
 for i = 0, character:military_force():unit_list():num_items() - 1 do 
   ...
   for i,value in pairs(new_units) do              
    if value ~= nil then
     ...
     -- PopLog("Remove pop for cqi: "..cqi, "AIRemovePop()")
      RemovePop(unit_key, unit_to_pop_table[unit_key] and unit_to_pop_table[unit_key][3] or 0, regionName, character, factionName)
      end
   end
end
...
local regionName = character:region():name()
 ...
 for i = 0, force:unit_list():num_items() - 1 do
   local unit = force:unit_list():item_at(i)
   local unit_key = unit:unit_key()
   if not AI_recruitment_table[tostring(cqi)] then AI_recruitment_table[tostring(cqi)] = {} end
...
if not faction:is_human() then -- maybe time trigger ?
...
local function checkForCharacterMovement()
  ...
  if stop_tick_tock == false then 
      player_factions = GetPlayerFactions()
        for i,faction in pairs(player_factions) do
...
local function tickTrigger(context)
  if stop_tick_tock == false then
        if context.string == "tick_tack"..triggerload then
...
if not currentFaction:is_human() then return end 
...
local function ArmyStatePendingBattle(context)
        ...
        if not attacker:military_force():general_character():faction():is_human() or defender:military_force():general_character():faction():is_human() then return end 
        ...
        for i = 0, armies - 1 do
            ...
            Log("armyKey: "..armyKey)
                ...
                if army:is_army() or army:is_navy() then
                    ...
                    if not armyState[armyKey] then armyState[armyKey] = createArmyList(army) end
...
local function ArmyStateFactionTurnEnd(context)
        -- removed human-only guard so AI also processed
        ...
        for i = 0, armies - 1 do
                ...
                if army:is_army() or army:is_navy()     
                  then updateArmy(armyCQI) -- Now we use armyState to determine changes which happen throughout the end turn sequence
                end
        end
end
...
function updateArmy(armyCQI, regionName)
  ...
  -- if the army list is pre crash fix we delete the full army table 
        ...
        if not character:has_region() then 
          ...
          regionName = getClosestPortRegion(x, y, character:faction()) end;
        ...
        if not armyState[armyKey] then armyState[armyKey] = {} end
        ...
        for unitKey, tab in pairs(difference) do
                ...
                if soldierCount < 0 and unitCount < 0 then -- A unit is missing, and the soldier count has dropped. Assume disbanding
                ...
                elseif soldierCount < 0 and unitCount >= 0 then -- No missing units, and loss in soldier count. Assume attrition
                              --  Log("Attrition in " .. unitKey .. ", killing " .. -soldierCount .. " soldiers")
                                -- No population change
                elseif soldierCount > 0 then -- Soldiers have been created (via replenishment or recruitment) and pop must be changed accordingly
...
function getArmyDifference(army1_table, army2_table)
        ...
        for unit_key, tab in pairs(army2_table) do
...
function createArmyList(army)
   PopLog("Create Army List")
        ...
        for i = 0, armyList:num_items() - 1 do
                ...
                if not unit_to_pop_table[unitKey] then unitSize = 120
                  elseif unit_to_pop_table[unitKey] then unitSize = unit_to_pop_table[unitKey][2]
                end
...
Log("Go through army list, CheckForNewCqi")
    for i = 0, armylist:num_items() -1 do
     local army = armylist:item_at(i)
    -- make sure we don't add garrison armies
       -- if not army:general_character():character_type("colonel") then
        ...
        -- if not army:general_character():character_type("colonel")
         ...
         Log("Go through army list: "..armyKey)
         cqi_list[armyKey] = true
         Log("Faction CQI Army list, add to cqi_list table "..armyKey)
            if not armyState[armyKey] then 
             not_cqi_list[armyKey] = true
             new_cqi = true
             Log("Army does not exist, add to not_cqi_list table "..armyKey)
            end
        end
    end
...
-- I need to find the army in the army state list that is not in the faction list
    ...
    Log("Check if Army State cqi is saved: "..k)
     local cqi = k
      if armyStateRegion[cqi][2] == factionName then
           if cqi_list[cqi] then
            Log("Saved Army already exists: "..cqi)
           end
          if not cqi_list[cqi] then
          not_in_old_list[cqi] = true
          old_cqi = true 
          Log("Char was removed from saved army, check for replacements for: "..cqi)
          end
      end
    end

  if old_cqi == true and new_cqi == true then
    Log("Search for army to copy")
 ...
 if old_cqi then
   ...
   Log("Disband Army oldcqi: "..oldcqi)
...
-- I need to find the army in the army state list that is not in the faction list
Log("Start armyState check")
  if armyState[tostring(cqi).."_"..factionName] then
   Log("RemoveDeadCqi:"..cqi)
...
function RemovePop(unitKey, soldierCount, regionName, character, factionName)
  PopLog("RemovePop")
    local class = 0
        ...
        for v, unit in pairs(mercenary_units_table) do
            if unit == unitKey 
                then class = Mercs_Levies_GetClass(unitKey, character)
            break end
        end
...
-- full disband currently won't work with mercenaries
...
local function ArmyStateTimeTrigger(context)
        if string.find(context.string, "armyMerged_") then
...
function resetUIPopulation()
  Log("Start Set UI POPULATION")
    UIPopulation = deepCopy(region_table)
    Log("End Set UI POPULATION")
end
...
--local function DebugComponent(context)
--local component = UIComponent(context.component):Id()
--Debug("context.string: "..tostring(component)) 
--local tooltip = UIComponent(context.component):GetTooltipText()
--Debug("UIComponent(context.component):GetTooltipText(): "..tooltip) 
--end
--scripting.AddEventCallBack("ComponentLClickUp", DebugComponent)
...
Log("Shortcut Triggered: " .. context.string);
...
-- === Patched recruitment listener functions (stable 3.5 behavior) ===
function RecListenerRemovePop(regionName, unitKey, factionName)
  local class = UIRetrieveClass(unitKey, factionName)
  local costs = RecListenerRetrieveunitcosts(unitKey, class)
  UIPopulation[regionName][class] = UIPopulation[regionName][class] - costs
